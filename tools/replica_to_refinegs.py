...
def main():
    ...
    if HAS_O3D and args.voxel > 0:
        ...
        if len(pcd.points) > args.max_points:
            idx = np.random.choice(len(pcd.points), args.max_points, replace=False)
            pcd = pcd.select_by_index(idx)
        o3d.io.write_point_cloud(os.path.join(sparse_dir, "points3D.ply"), pcd)
        print(f"voxel({args.voxel}m) 다운샘플 후: {len(pcd.points)} 점 → points3D.ply")
    else:
        if xyz.shape[0] > args.max_points:
            idx = np.random.choice(xyz.shape[0], args.max_points, replace=False)
            xyz, rgb = xyz[idx], rgb[idx]
        write_ply(os.path.join(sparse_dir, "points3D.ply"), xyz, rgb)
        print(f"points3D.ply 기록: {xyz.shape[0]} 점 (open3d 없어 voxel 생략)")
