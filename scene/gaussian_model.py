...
class GaussianModel:
    ...
    def compute_color(self, camera):
        ...
        # 최종 색상
        color = albedo * shading  # [N, 3] * [N, 1] → broadcast → [N, 3]
    ...
    def training_setup(self, training_args):
        ...
        l = [
            ...
            # add albedo and shading parameters 
    ...
    def save_ply(self, path):
        ...
        attributes = np.concatenate((xyz, normals, albedo, opacities, scale, rotation), axis=1)
    ...
    def load_ply(self, path):
        ...
        assert len(albedo_names) == 3, f"Expected 3 albedo channels, got {len(albedo_names)}"
        ...
        # self._features_rest = nn.Parameter(tor
...
class ShadingMLP(nn.Module):
    ...
    def forward(self, normal, position, view_dir):
        """
        Args:
            normal:   [N, 3] 가우시안 법선 벡터 (정규화됨)
            position: [N, 3] 가우시안 3D 위치
            view_dir: [N, 3] 카메라→가우시안 방향 벡터 (정규화됨)
        Returns:
            shading:  [N, 1] shading factor
        """
